...
class Autoencoder:
    def __init__(self, name, X_train, X_test, Y_train, Y_test, epochs, batch_size):
        self.name = name
    ...
    def train(self):
        ...
        self.history = self.autoencoder.fit(self.X_train, self.Y_train, epochs=self.epochs, batch_size=self.batch_size, 
        shuffle=True, validation_data=(self.X_test, self.Y_test), callbacks=self.callbacks)
    def save(self):
